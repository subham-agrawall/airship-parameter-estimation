...
# plt.figure(1)
# plt.plot(range(len(dataset)),dataset['Cm_act'],'--r')
...
estimates=pd.DataFrame(index=range(-15,19,1),columns=['cm0','cm_q','cm_de'])
...
estimates['cm0']=cl0_output.reshape(len(estimates),1)
...
estimates['cm_q']=clq_output.reshape(len(estimates),1)
...
estimates['cm_de']=cld_output.reshape(len(estimates),1)
...
plt.savefig('./result6/cm.eps')
plt.show()

# # serialize model to JSON
# model_json = model.to_json()
# with open("./models/cm.json", "w") as json_file:
#     json_file.write(model_json)

# # serialize weights to HDF5
# model.save_weights("./models/cm.h5")
# print("Saved model to disk")

# load json and create model
json_file = open('./models_1/cm.json','r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
loaded_model.load_weights("./models_1/cm.h5")
